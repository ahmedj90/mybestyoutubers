...
function snax_get_poll_share_links( $poll_id, $question_id, $answer_id ) {
	...
	if ( ! $question['title_hide'] ) {
		$snax_share_args['description'] = $question['title'] . esc_attr( ' My choice: ', 'snax' ) . $answer['title'] . esc_attr( '. Play!', 'snax' );
	} else {
		$snax_share_args['description'] = esc_attr( ' My choice: ', 'snax' ) . $answer['title'] . esc_attr( '. Play!', 'snax' );
	}

	$snax_share_args['url'] = $url;
	if ( $question['media']['id'] ) {
		$image = wp_get_attachment_image_src( $question['media']['id'], 'full' )[0];
		$snax_share_args['thumb'] = $image;
	}
	if ( 'versus' === $poll_type && $question['answers'][0]['media']['id'] ) {
		$image = wp_get_attachment_image_src( $question['answers'][0]['media']['id'], 'full' )[0];
		$snax_share_args['thumb'] = $image;
	}
