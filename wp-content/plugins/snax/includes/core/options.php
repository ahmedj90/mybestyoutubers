...
/**
 * Check whether to make featured images mandatory
 *
 * @return bool
 */
function snax_featured_media_required() {
	return 'standard' === apply_filters( 'snax_featured_media_required', get_option( 'snax_featured_media_required', 'none' ) );
}
...
/**
 * Return formats where showing featured images is enabled.
 *
 * @param array   $default Optional. Default value.
 * @param WP_Post $post Optional. Default value.
 *
 * @return array
 */
function snax_show_featured_images_for_formats( $default = array( 'text', 'trivia_quiz', 'personality_quiz' ), $post = null ) {
	$post = get_post( $post );

	$formats_ids = (array) get_option( 'snax_show_featured_images_for_formats', $default );

	return apply_filters( 'snax_show_featured_images_for_formats', $formats_ids, $post );
}

/**
 * Check whether to allow snax_autor to add referral links to posts and items
 *
 * @return bool
 */
function snax_allow_snax_authors_to_add_referrals() {
	return 'standard' === apply_filters( 'snax_allow_snax_authors_to_add_referrals', get_option( 'snax_allow_snax_authors_to_add_referrals', 'standard' ) );
}
